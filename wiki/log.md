...
2026-07-12 20:09 UTC — Phase 28 dispatch | tier=local | "create a snake game for my phone" | 158.9s | $0.0000 | done

2026-07-12 19:43 UTC — Phase 28 dispatch | tier=local | "e2e: all-local tier smoke test" | 63.3s | $0.0000 | done

2026-07-12 19:28 UTC — Phase 28 dispatch | tier=max | "Create the game flappy bird so I can play on my ph" | 11.1s | $0.0000 | failed

2026-06-29 20:40 PDT — Phase 42 shipped: Telegram Rich Messages (Bot API 10.1). Verified empirically (method-specific 400 not 404, + docs) that sendRichMessage/sendRichMessageDraft exist; rich_message is just {"markdown": ...} (bogus-chat_id probe → "chat not found" = content accepted), so the model's markdown renders natively (headings/tables/code/lists, CommonMark — `##` and `|` tables, no escaping) with NO block-converter. PTB 22.7 has no typed support → isolated raw httpx POST. Part A (default ON): _reply_smart routes final quick_chat replies (handle_message, /quick, streamed finalize) through sendRichMessage when rich constructs present and ≤32768 chars; any rejection/>32k/plain-prose falls back to the Phase 41 plain chunker (kept as overflow path), reply never dropped. Part B (default OFF, TELEGRAM_RICH_DRAFTS): _send_rich_draft via sendRichMessageDraft; draft mechanism chosen once per message (rich xor plain), never mixed in a lifecycle, never uses editMessageText (avoids the formatting-destruction bug class others hit). Live reproducer msg 1061 renders a native table. evals 34/34 exit 0; +9 tests (rich), Phase 41 (14) + dispatch (30) green.

2026-06-29 20:12 PDT — Phase 41 shipped: Telegram Smart Reply (chunking + native draft streaming). Real cause of the cut-off MoE reply was the 120-token qwen-tuned content budget capping the Ornith brain at ~980 chars — NOT the 4096 limit; raised brain budget to 1024 (qwen3:4b stays 120). New core/telegram_chunk.py is the one shared chunker (cc_result_reporter delegates, 30 tests green); chunk_text() adds paragraph→sentence→word fallback so long prose splits below 4096. handle_message + /quick + slash dispatch now chunk instead of truncating at 4000. Part B: quick_chat_stream() + _stream_quick_chat_reply() push live sendMessageDraft (PTB 22.7 native, throttled ~1s) for pure chat turns, finalize with chunked sendMessage; draft failure → typing+final, generation error → blocking fallback, reply never dropped. evals 34/34 exit 0; +14 new tests. Brain-swap fallout (6 hardcoded gpt-oss assertions in test_quick_chat_fallback.py) tracked separately.
